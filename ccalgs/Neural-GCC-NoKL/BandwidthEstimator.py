...
# -*- coding: utf-8 -*-
"""
Neural-GCC-NoKL 带宽估计器
消融实验变体2：有 BC-GCC 预训练，无 KL 约束
- 使用预训练的 BC-GCC 模型作为初始策略
- 不使用 KL 散度约束（或可调节 KL 权重）
- 允许 PPO 更大幅度地偏离预训练策略
"""
import sys
import os
...
class Estimator(object):
    """Neural-GCC-NoKL 带宽估计器 (BC-GCC + PPO，无 KL 约束)"""
    ...
    def __init__(self, model_path="/home/wyq/桌面/mininet-RTC/ccalgs/BC-GCC/trial3.pt", 
        ...
        Args:
            ...
            use_bc_model (bool): 是否使用 BC-GCC 预训练模型（NoKL 版本固定为 True）
            kl_weight (float): KL 约束权重（NoKL 版本默认为 0，可调节）
        """
        ...
        logging.basicConfig(level=logging.INFO)
        global logger
        logger = logging.getLogger("NeuralGCC-NoKL")
    ...
    def _update_policy(self):
        ...
        for _ in range(5):
            ...
            # Total Loss: 使用可调节的 KL 权重（默认为 0，即无 KL 约束）
            loss = actor_loss + 0.5 * critic_loss - 0.01 * entropy + self.kl_weight * kl_div
