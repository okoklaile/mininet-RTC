...
class VideoMetrics:
    ...
    @staticmethod
    def calculate_metrics(video_stats):
        """
        计算各种视频质量指标
        ...
        aggregated = {
            ...
            'total_freeze_duration': time_series['freeze_dur_ms'][-1] if time_series['freeze_dur_ms'] else 0,
...
def generate_report(data_dict, output_path):
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        f.write("=" * 100 + "\n")
        f.write("\n评分说明:\n")
        f.write("  - QoE评分综合考虑: 帧率稳定性、延迟、卡顿、丢包等因素\n")
...
def calculate_qoe_score(aggregated):
    ...
    # 4. 丢包评分 (满分20)
    loss = aggregated['total_packet_loss']
    if loss == 0:
        score += 20
    elif loss <= 10:
        score += 15
